...
def test_logs_localtime_flag_markdown(log_path, monkeypatch):
    ...
    result = runner.invoke(
        cli, ["logs", "-p", str(log_path), "-L", "-n", "1"], catch_exceptions=False
    )
    assert result.exit_code == 0
    output = result.output
    ...
    assert datetime_re.search(
        output
    ), "Datetime format should match YYYY-MM-DDTHH:MM:SS"
...
def test_logs_localtime_flag_json(log_path):
    ...
    result = runner.invoke(
        cli,
        ["logs", "-p", str(log_path), "-L", "--json", "-n", "1"],
        catch_exceptions=False,
    )
    assert result.exit_code == 0
    logs = json.loads(result.output)
...
def test_logs_default_utc_json(log_path):
    ...
    # Test without -L flag
    result = runner.invoke(
        cli, ["logs", "-p", str(log_path), "--json", "-n", "1"], catch_exceptions=False
    )
    assert result.exit_code == 0
    logs = json.loads(result.output)
...
def test_logs_localtime_config_file(user_path, monkeypatch):
    ...
    runner = CliRunner()
    result = runner.invoke(
        cli, ["logs", "-p", str(log_path), "-n", "1"], catch_exceptions=False
    )
    assert result.exit_code == 0
